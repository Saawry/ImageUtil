...
//
//Share
//I am making a image cropper project. the github link is given bellow. when I try to capture image using camera, getting the error
//android.os.FileUriExposedException: file:///storage/emulated/0/Android/data/com.gadware.android.cropimage/cache/data/user/0/com.gadware.android.cropimage/cache/tmp_image_file8595149629970255016.png exposed beyond app through ClipData.Item.getUri()...........
//repo: https://github.com/Saawry/ImageCropper
//
//Lite
//I've received your request regarding the FileUriExposedException in your ImageCropper project. I will analyze the repository and provide a fix for this issue.
//Cloned the repository and will search for camera intents or file URIs to identify where the FileUriExposedException occurs for fixing.
//
//I found camera-related intent usage and file URIs in Kotlin files. Will review these areas to identify where FileUriExposedException occurs and plan the fix with FileProvider.
//
//Reviewing code, I see camera Uri is passed via intent but not set up with FileProvider, which likely causes FileUriExposedException. Next, I will implement FileProvider to handle Uri sharing securely.
//
//I've confirmed the camera Uri is passed via intent but not set up with FileProvider. Next step: implement FileProvider in manifest and code to prevent FileUriExposedException.
//
//I fixed the authority mismatch in GetUriForFile.kt and will now review BitmapUtils.kt for any Uri.fromFile usages to ensure proper FileProvider implementation.
//
//Fixed Uri.fromFile for SDK < 29 in BitmapUtils.kt to prevent FileUriExposedException; confirmed proper usage with getUriForFile. Next, will review GetFilePathFromUri.kt for similar issues.
//
//Lite
//I have fixed the android.os.FileUriExposedException in your ImageCropper project.
//Root Cause Analysis
//The error occurred because the app was attempting to share a file:// URI with the camera app, which is prohibited on Android 7.0 (API 24) and higher. While your project already had a FileProvider configured, there was a mismatch between the authority declared in the AndroidManifest.xml and the one used in the code:
//Manifest: ${applicationId}.fileprovider
//Code: "$packageName.cropper.fileprovider"
//Because of this mismatch, FileProvider.getUriForFile() would fail, and the code would fall back to using Uri.fromFile(), triggering the exception.
//Changes Made
//GetUriForFile.kt: Updated the authority() extension function to return "$packageName.fileprovider", matching the manifest declaration.
//BitmapUtils.kt: Removed the conditional check that used Uri.fromFile() for SDK versions below 29. Now, it consistently uses the getUriForFile() helper, ensuring all temporary files are shared via content:// URIs through the FileProvider.
//How to Apply
//Replace the following files in your project with the attached versions:
//app/src/main/java/com/gadware/android/cropimage/GetUriForFile.kt
//app/src/main/java/com/gadware/android/cropimage/BitmapUtils.kt
//After applying these changes, the camera capture should work correctly without the FileUriExposedException.
//GetUriForFile.kt
//Code · 3.22 KB
//BitmapUtils.kt
//Code · 30.21 KB
...
import com.gadware.android.cropimage.CropException
import com.gadware.android.cropimage.getUriForFile
...
import com.gadware.android.cropimage.CropImageView.RequestSizeOptions
import java.io.File
